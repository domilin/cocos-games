...
import { BaseModuleEvent, CompleteCallBack, EModuleType, EUILayer, Maths, RandomSeed, TResoucesUrl, _config_, _login, _platform, _prop, _resouces, _scene, _timer, _ui, moduleMgr } from "../Main";
...
export class GameTypeModule extends BaseModuleEvent {
    ...
    public onceEnter(once: boolean) {
        if (once)
            if (this.isOnce)
                return false
        return _login.isNewUser && initData.newUseEnterGame
    }
